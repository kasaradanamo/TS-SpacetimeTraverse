...
public record PortalActionController(PortalActionScreen screen) {
    ...
    public void onOpenWaypointFormRegister() {
        Minecraft.getInstance().gui.setScreen(WaypointFormScreen.register(screen));
    }
    ...
    public void onEditWaypoint(WaypointListWidget.@Nullable Entry selected) {
        if (selected == null) return;
        Minecraft.getInstance().gui.setScreen(WaypointFormScreen.edit(screen, selected.data));
    }
    ...
    public void onDeleteWaypoint(WaypointListWidget.@Nullable Entry selected) {
        ...
        Minecraft.getInstance().gui.setScreen(new ConfirmScreen(
                confirmed -> {
                    ...
                    Minecraft.getInstance().gui.setScreen(screen);    // 元の画面に戻す
                },
