...
public class PortalRenderer extends EntityRenderer<PortalEntity> {
    ...
    private static final ResourceLocation TEXTURE = ResourceLocation.fromNamespaceAndPath(TokorotenSlimeAPI.getModId(), "textures/entity/portal.png");
    ...
    private void drawPortal(PoseStack poseStack, MultiBufferSource buffer, float alpha) {
        ...
        // 頂点を指定して四角形を描画
        vc.addVertex(mat, -WIDTH / 2, 0, 0).setColor(255, 255, 255, a).setUv(0, 1).setOverlay(overlay).setLight(light).setNormal(pose, 0, 0, 1);
        vc.addVertex(mat, WIDTH / 2, 0, 0).setColor(255, 255, 255, a).setUv(1, 1).setOverlay(overlay).setLight(light).setNormal(pose, 0, 0, 1);
        vc.addVertex(mat, WIDTH / 2, HEIGHT, 0).setColor(255, 255, 255, a).setUv(1, 0).setOverlay(overlay).setLight(light).setNormal(pose, 0, 0, 1);
        vc.addVertex(mat, -WIDTH / 2, HEIGHT, 0).setColor(255, 255, 255, a).setUv(0, 0).setOverlay(overlay).setLight(light).setNormal(pose, 0, 0, 1);
    }
