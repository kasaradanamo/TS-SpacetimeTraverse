...
public class PortalActionClientHandler {
     ...
     */
    public static void placePortal(Player player, @Nullable UUID waypointUuid) {
        // Waypoint未指定の場合はエラーメッセージ表示
        if (waypointUuid == null) {
            player.displayClientMessage(Component.translatable("message.tokorotenslime.not_place_portal"), true);
            return;
        }
        // サーバーに設置要求
